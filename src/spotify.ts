...
export const search = async (q, type = 'track'): Promise<SearchResponse> => fetch('/v1/search', {
  query: { q, type },
});
...
export const queueAdd = async (uri: string) => fetch('/v1/me/player/queue', {
  method: 'POST',
  query: { uri },
});

export const currentlyPlaying = async () => fetch('/v1/me/player/currently-playing');
...
): Promise<RecommendationsResponse> => fetch('/v1/recommendations', {
  query: {
...
export const listPlaylists = async (
  limit: number = 50,
): Promise<MyPlaylistsResponse> => fetch('/v1/me/playlists', { query: { limit } });

export const playlistAdd = async (playlistId: string, uris: string[]) => fetch(`/v1/playlists/${playlistId}/tracks`, {
  method: 'POST',
  body: { uris },
});
